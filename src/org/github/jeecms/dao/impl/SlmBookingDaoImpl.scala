...
class SlmBookingDaoImpl() extends HibernateBaseDao[SlmBooking, Integer] with SlmBookingDao {
  ...
  def getPage(userId: Option[Integer], contentId: Option[Integer], siteId: Option[Integer],
    ...
    userId.foreach(t => f.append(" and booking.user.id = :userId ").setParam("userId", t))
    contentId.foreach(t => f.append(" and booking.content.id = :contentId ").setParam("contentId", t))
    siteId.foreach(t => f.append(" and booking.content.site.id = :siteId ").setParam("siteId", t))
    f.append(" and exists(select 1 from Content where id = booking.content.id)")
    f.append(" and booking.status != 1 ")
    f.append(" order by booking.status ")
